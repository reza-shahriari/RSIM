...
### ▶️ `ControlVariates`
> Implement **control variates variance reduction** technique for Monte Carlo simulations, using auxiliary random variables with known expectations to dramatically reduce estimation variance while maintaining unbiasedness.

<details>
  <summary>Click to expand – Full explanation, theory, and examples</summary>

#### 📘 What it does
This class implements the **control variates method**, one of the most powerful variance reduction techniques in Monte Carlo simulation. It exploits correlation between the target estimator and auxiliary variables (control variates) with known expected values to create new estimators with significantly lower variance.

#### 🎯 Built-in Examples
- **Exponential Integration**: ∫₀¹ e^x dx with control ∫₀¹ x dx = 0.5
- **Asian Option Pricing**: Arithmetic mean option with geometric mean control
- **Normal CDF Estimation**: P(Z > x) with linear approximation control
- **Gamma Function**: Γ(x) estimation with Stirling approximation
- **Portfolio VaR**: Portfolio risk with individual asset controls

#### 📐 Supported Methods
- **Single Control Variate**: One auxiliary variable with optimal coefficient
- **Multiple Control Variates**: Regression-based combination of several controls
- **Automatic Coefficient Estimation**: Optimal variance-minimizing coefficients
- **Bootstrap Confidence Intervals**: Robust statistical inference

#### 📊 Supported Distributions
- Uniform, Normal, Exponential, Log-normal
- Multivariate Normal (for portfolio/network problems)
- Custom distributions via callable functions

#### 📚 Theoretical Background

**Control Variate Estimator:**
Let X be the target random variable and Y be a control variate with known E[Y] = μ_Y.
...
For controls Y₁, Y₂, ..., Yₖ:
```
X_cv = X - Σᵢ cᵢ(Yᵢ - E[Yᵢ])
```

Optimal coefficients: **c = Σ_Y⁻¹ · σ_XY**

#### ✅ Properties
- **Unbiased**: E[X_cv] = E[X] regardless of control coefficient
- **Variance Reduction Factor**: 1 - ρ² (up to 99% for ρ = 0.99)
- **Efficiency Gain**: ~1/(1-ρ²) effective sample size increase
- **Computational Overhead**: Only 10-20% vs standard Monte Carlo

#### 📈 Example – Built-in exponential integral
```python
from RSIM.variance_reduction.control_variates import ControlVariates

# Estimate ∫₀¹ e^x dx ≈ 1.718 using ∫₀¹ x dx = 0.5 as control
...
print(f"Estimate: {result.results['cv_estimate']:.6f}")
print(f"95% CI: [{result.results['cv_ci_lower']:.6f}, {result.results['cv_ci_upper']:.6f}]")
```

#### 📉 Example – Multiple control variates
```python
# Portfolio VaR with multiple asset controls
cv_multi = ControlVariates('portfolio_var', multiple_controls=True)

# Add additional control variates
def control_func2(returns):
    return np.var(returns, axis=1)  # Portfolio variance

def control_func3(returns):
    return np.max(returns, axis=1)  # Maximum individual return

cv_multi.add_control_variate(control_func2, known_mean2)
cv_multi.add_control_variate(control_func3, known_mean3)
...
```python
# Asian call option with geometric mean control
cv_asian = ControlVariates(
    target_function='asian_option',
    n_samples=100000,
    random_seed=42
)

result = cv_asian.run()
print(f"Asian option price: {result.results['cv_estimate']:.4f}")
print(f"Standard error reduction: {result.results['cv_se'] / result.results['standard_se']:.2f}x")

# Advanced analysis
bootstrap_ci = cv_asian.bootstrap_confidence_intervals(n_bootstrap=1000)
print(f"Bootstrap 95% CI: [{bootstrap_ci['control_variates']['ci_lower']:.4f}, "
      f"{bootstrap_ci['control_variates']['ci_upper']:.4f}]")
```

#### 📊 Variance Reduction Guidelines

| Correlation |ρ| | Variance Reduction | Efficiency Gain | Quality |
|-------------|-----|-------------------|-----------------|---------|
| 0.3         | 9%  | 1.1x             | Minimal         |
| 0.5         | 25% | 1.3x             | Good            |
| 0.7         | 51% | 2.0x             | Very Good       |
| 0.8         | 64% | 2.8x             | Excellent       |
| 0.9         | 81% | 5.3x             | Outstanding     |

#### 🎛️ Advanced Features

**Convergence Tracking:**
```python
cv_sim = ControlVariates('exponential_integral', show_convergence=True)
result = cv_sim.run()

# Plot convergence
import matplotlib.pyplot as plt
samples = [point[0] for point in result.convergence_data]
cv_estimates = [point[2] for point in result.convergence_data]
plt.plot(samples, cv_estimates, label='Control Variates')
...
optimal_coeff = sensitivity['optimal_coefficient']
print(f"Optimal coefficient: {optimal_coeff:.4f}")
print(f"Coefficient sensitivity: {sensitivity['optimal_variance']:.6f}")
```

**Sample Size Planning:**
```python
required = cv_sim.estimate_required_samples(target_error=0.001, confidence_level=0.95)
print(f"Standard MC needs: {required['standard_mc_samples']:,} samples")
print(f"Control Variates needs: {required['control_variates_samples']:,} samples")
print(f"Sample reduction: {required['sample_reduction']:.1f}x")
```

#### 🏭 Applications

**Financial Engineering:**
- Option pricing (underlying as control)
...
| Aspect | Standard MC | Control Variates |
|--------|-------------|------------------|
| Time Complexity | O(n) | O(n) |
| Memory Usage | n | 2-3n |
| Computational Overhead | - | +10-20% |
| Effective Sample Size | n | n/(1-ρ²) |
...
ControlVariates(
    ...
    control_mean=None,                       # Known E[Y] for custom controls
    n_samples=100000,                        # Number of MC samples
    distribution='uniform',                  # Sampling distribution
    distribution_params={},                  # Distribution parameters
    multiple_controls=False,                 # Use multiple controls
